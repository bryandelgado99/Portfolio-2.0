...
export const EducationSection = () => {
  const { t } = useLanguage();
  const education = t.education.items.map((edu, i) => ({
    degree: edu.degree,
    school: edu.school,
    year: edu.year,
    badge: i === 0 ? "Tecnología Superior" : "Bachillerato",
    badgeColor:
      i === 0
        ? "text-primary bg-primary/10 border-primary/30"
        : "text-emerald-400 bg-emerald-500/10 border-emerald-500/30",
    icon: i === 0 ? GraduationCap : BookOpen,
    details: edu.details,
  }));
      ...
      <div>
        ...
        >
          <div className="flex items-center gap-4 mb-3">
            ...
            <h2 className="text-3xl md:text-5xl font-extrabold tracking-tight">
              {t.education.heading} <span className="text-gradient-primary">{t.education.headingAccent}</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {education.map((edu, i) => {
            ...
            const BadgeIcon = edu.badgeColor.startsWith("text-primary") ? Sparkles : Award;
            return (
              <motion.div
                key={i}
                initial={{ opacity: 0, y: 30 }}
              ...
              >
                <MagicCard
                  ...
                  gradientSize={300}
                  gradientColor="rgba(220, 38, 38, 0.12)"
                  gradientFrom="#dc2626"
                  gradientTo="#f97316"
                >
