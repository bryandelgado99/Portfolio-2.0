      ...
      {showHeader && (
        ...
        >
          <div className="glass-panel w-full max-w-7xl rounded-b-2xl flex items-center justify-between px-6 py-4 shadow-xl">
            ...
            >
              <div className="relative w-9 h-9 rounded-xl bg-gradient-to-tr from-red-600 via-primary to-rose-500 p-[1px] shadow-lg group-hover:scale-105 transition-transform duration-300">
            ...
            <nav className="hidden md:flex flex-1 justify-center">
              <ul className="flex space-x-8">
                {navItems.map((item) => (
                  <motion.li key={item.href} className="relative group text-sm font-medium text-muted-foreground transition-colors">
                    <a onClick={() => handleScrollTo(item.href)} className="cursor-pointer hover:text-foreground">
                      {item.name}
                    </a>
                    <motion.span
                      className="absolute -bottom-2 left-1/2 w-0 h-0.5 bg-primary/80 rounded-full shadow-[0_0_8px_rgba(220,38,38,0.8)]"
              ...
              <div className="flex items-center rounded-full glass-panel border border-foreground/10 p-1">
                {(["es", "en"] as const).map((code) => (
                  <button
                    ...
                    className={`text-[11px] font-bold uppercase px-2 py-1 rounded-full transition-all cursor-pointer ${
                      lang === code
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (
              ...
              >
                ...
                >
                  {navItems.map((item) => (
                    <motion.li key={item.href} {...({ variants: itemVariants } as MotionProps)}>
                      <a
